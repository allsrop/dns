...
rdedns.controller('DNSCtrl', function ($scope, $http, $filter, $mdSidenav, $mdDialog, $mdToast) {
	...
	//Cara ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓modifyDomain
	$scope.modDomain = function(ev, domain) {
		$mdDialog.show({
			locals: {domain: domain},
			controller: DomainDialogCtrl,
			templateUrl: 'modifyDomain.html',
			targetEvent: ev
		}).then(function(domain_name) {
			if (! domain_name) return;
			var data = {id: domain.id, domain: domain_name};
			$http.post('d/modify', data).success(function(data) {
				var msg_part = 'failed: ' + data.message + '.';
				if (data.result) {
					msg_part = 'modified.';
	...
	//Cara ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓modifyRecord
	$scope.modRecord = function(ev, domain_obj, record) {
		$mdDialog.show({
			locals: {domain: record},
			controller: RecordDialogCtrl,
			templateUrl: 'modifyRecord.html',
			targetEvent: ev
		}).then(function(obj) {
			if (! obj) return;
			obj.domain = domain_obj.id;
			obj.id = record.id;
			$http.post('r/modify', obj).success(function(data) {
				var msg = 'Failed to modify record: ' + data.message + '.';
				if (data.result) {
					msg = 'Record is modified.';
					domain_obj.records = $filter('filter')(domain_obj.records, function(v) {
	...
	var DomainDialogCtrl = function($scope, $mdDialog, domain) {
		$scope.domain = domain;
